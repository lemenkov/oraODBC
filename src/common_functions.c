...
#elif defined(WIN32)
	return (int)(HANDLE_MUTEX(handle) =
		     (int *)CreateMutex(NULL, FALSE, NULL));
#else
	return 1;
#endif
}
...
static void _log_message_imp(char *tracefilename, char *file, int line,
			     int mask, SQLHANDLE handle, SQLRETURN ret,
			     char *fmt, va_list ap)
{
		     ...
		     SQLHANDLE handle, SQLRETURN ret, char *fmt, ...)
{
	...
	while (EBUSY == pthread_mutex_trylock(&((hgeneric *) dbc)->mutex)) {
		fprintf(stderr,
			"ood_log_message -> dbc [0x%.8lx] on pid %d locked\n",
			(long)dbc, getpid());
		sleep(1);
		if (i++ > 10)
			abort();
	 ...
	 */
	va_start(ap, fmt);
	_log_message_imp((char *)dbc->tracefile, file, line, mask, handle, ret,
			 fmt, ap);
	va_end(ap);
	THREAD_MUTEX_UNLOCK(dbc);
	return;
