...
sword ood_fetch_sqlcolumns(struct hStmt_TAG *stmt)
{
	...
	if (status != SQL_NO_DATA && stmt->bookmark) {
		...
		ret = OCIStmtFetch_log_stat(stmt->oci_stmt, stmt->dbc->oci_err,
					    stmt->row_array_size,
					    OCI_FETCH_NEXT, OCI_DEFAULT, ret);
		...
		if (ret == OCI_NO_DATA) {
			...
			stmt->num_fetched_rows =
			    stmt->num_result_rows % stmt->row_array_size;
			     ...
			     SQLCHAR * ColumnName, SQLSMALLINT NameLength4)
{
	...
	char sql[512] =
	    "SELECT NULL, '', TABLE_NAME, COLUMN_NAME, DATA_TYPE, DATA_TYPE, NVL(DATA_PRECISION,DATA_LENGTH), DATA_LENGTH,DATA_SCALE, 10, REPLACE(REPLACE(NULLABLE,'Y',1),'N',0), ' ', NULL, DATA_TYPE, NULL, CHAR_COL_DECL_LENGTH, COLUMN_ID, REPLACE(REPLACE(NULLABLE,'N','NO'),'Y','YES') FROM USER_TAB_COLUMNS";
#else
	char sql[512] =
	    "SELECT NULL, OWNER, TABLE_NAME, COLUMN_NAME, DATA_TYPE, DATA_TYPE, NVL(DATA_PRECISION,DATA_LENGTH), DATA_LENGTH,DATA_SCALE, 10, REPLACE(REPLACE(NULLABLE,'Y',1),'N',0), ' ', NULL, DATA_TYPE, NULL, CHAR_COL_DECL_LENGTH, COLUMN_ID, REPLACE(REPLACE(NULLABLE,'N','NO'),'Y','YES') FROM ALL_TAB_COLUMNS";
#endif
	...
	if (getenv("VERBOSE"))
		fprintf(stderr,
			"SQLColumns schema [%s], table [%s], column [%s]\n",
			schema, table, column);
	...
	if (table && *table) {
		if (has_where_clause)
			...
			sql_end =
			    ood_fast_strcat(sql, " WHERE TABLE_NAME", sql_end);
	...
	if (column && *column) {
		if (has_where_clause)
			sql_end =
			    ood_fast_strcat(sql, " AND COLUMN_NAME", sql_end);
		else
			sql_end =
			    ood_fast_strcat(sql, " WHERE COLUMN_NAME", sql_end);
	...
	if (ENABLE_TRACE) {
		ood_log_message(stmt->dbc, __FILE__, __LINE__,
				TRACE_FUNCTION_ENTRY, (SQLHANDLE) stmt, 0, "s",
				"SQL", sql);
	}
