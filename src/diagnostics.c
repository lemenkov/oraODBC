...
SQLRETURN _SQLGetDiagField(SQLSMALLINT HandleType,
			   SQLHANDLE Handle,
			   ...
			   SQLSMALLINT * StringLengthPtr)
{
	...
	case SQL_DIAG_ROW_COUNT:
		if (HandleType == SQL_HANDLE_STMT) {
			*(int *)DiagInfoPtr =
			    ((hStmt_T *) Handle)->num_result_rows;
			status = SQL_SUCCESS;
		}
	...
	default:
		{
			...
			case SQL_DIAG_CLASS_ORIGIN:
				if (!ood_bounded_strcpy((char *)DiagInfoPtr,
							(char *)ptr->origin,
							BufferLength)) {
					ood_post_diag(hGH, ERROR_ORIGIN_01004,
						      -1, "",
						      ERROR_MESSAGE_01004,
						      __LINE__, -1, "",
						      ERROR_STATE_01004,
			...
			case SQL_DIAG_CONNECTION_NAME:
				if (!ood_bounded_strcpy
				    (DiagInfoPtr, "Easysoft Oracle",
				     BufferLength)) {
					ood_post_diag(hGH, ERROR_ORIGIN_01004,
						      -1, "",
						      ERROR_MESSAGE_01004,
...
SQLRETURN _SQLGetDiagRec(SQLSMALLINT HandleType,
			 SQLHANDLE Handle,
			 SQLSMALLINT RecNumber,
			 SQLCHAR * SqlState,
...
/*        return SQL_ERROR;*/
	...
	if (TextLengthPtr)
		*TextLengthPtr =
		    ptr->message ? strlen((const char *)ptr->message) : 0;
		   ...
		   char *server_name, char *sqlstate, char *file, int line)
{
	...
	if (ENABLE_TRACE) {
		...
		case SQL_HANDLE_DBC:
			ood_log_message((hDbc_T *) hH, __FILE__, __LINE__,
					TRACE_FUNCTION_EXIT, (SQLHANDLE) NULL,
					0, "sis", "FILE", file, "LINE", line,
					"message", message);
			break;
		case SQL_HANDLE_STMT:
			ood_log_message(((hStmt_T *) hH)->dbc, __FILE__,
					__LINE__, TRACE_FUNCTION_EXIT,
					(SQLHANDLE) NULL, 0, "sis", "FILE",
					file, "LINE", line, "message", message);

			break;
		case SQL_HANDLE_DESC:
			ood_log_message(((hDesc_T *) hH)->dbc, __FILE__,
					__LINE__, TRACE_FUNCTION_EXIT,
					(SQLHANDLE) NULL, 0, "sis", "FILE",
					file, "LINE", line, "message", message);
			break;
