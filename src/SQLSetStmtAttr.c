				 ...
				 SQLPOINTER ValuePtr, SQLINTEGER StringLength)
{
	...
	if (ENABLE_TRACE) {
		ood_log_message(stmt->dbc, __FILE__, __LINE__,
				...
				"shi", "Attribute",
				odbc_sql_attr_type(Attribute), "ValuePtr",
				ValuePtr, "StringLength", StringLength);
	}
	...
	case SQL_ATTR_ROW_BIND_TYPE:
		if ((SQLINTEGER) ValuePtr == SQL_BIND_BY_COLUMN)
			...
			ood_post_diag((hgeneric *) stmt->dbc,
				      ERROR_ORIGIN_IM001, 0, "",
				      ERROR_MESSAGE_IM001, __LINE__, 0, "",
				      ERROR_STATE_IM001, __FILE__, __LINE__);
