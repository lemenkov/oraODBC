				  ...
				  SQLLEN *NumericAttributePtr)
{
	...
	if (ENABLE_TRACE) {
		ood_log_message(stmt->dbc, __FILE__, __LINE__,
				TRACE_FUNCTION_ENTRY, (SQLHANDLE) stmt, 0, "is",
				"ColumnNumber", ColumnNumber, "FieldIdentifier",
				odbc_desc_type(FieldIdentifier));
	}
...
//#define UNIX_DEBUG
		...
		fprintf(stderr, "SQL_DESC_AUTO_UNIQUE_VALUE=%d %s %d\n",
			*((SQLINTEGER *) NumericAttributePtr), __FILE__,
			__LINE__);
#endif
		break;
	...
	case SQL_COLUMN_NAME:
					...
					(char *)ar->column_name, BufferLength))
		{
			ood_post_diag((hgeneric *) stmt, ERROR_ORIGIN_01004,
				      ColumnNumber, "", ERROR_MESSAGE_01004,
			...
			*StringLengthPtr =
			    strlen((const char *)ar->column_name);
#ifdef UNIX_DEBUG
	...
	case SQL_DESC_LITERAL_PREFIX:
		...
		if (StringLengthPtr)
			*StringLengthPtr =
			    strlen((const char *)ar->literal_prefix);
		break;
	...
	case SQL_DESC_LITERAL_SUFFIX:
		...
		if (StringLengthPtr)
			*StringLengthPtr =
			    strlen((const char *)ar->literal_suffix);
		break;
	...
	case SQL_DESC_LOCAL_TYPE_NAME:
		...
		if (ar->local_type_name) {
			ood_bounded_strcpy(CharacterAttributePtr,
					   (char *)ar->local_type_name,
					   BufferLength);
			if (StringLengthPtr)
				*StringLengthPtr =
				    strlen((const char *)ar->local_type_name);
		} else {
	...
	case SQL_DESC_SCHEMA_NAME:
		...
		fprintf(stderr, "SQL_DESC_SCHEMA_NAME %s %d\n", __FILE__,
			__LINE__);
#endif
		ood_bounded_strcpy(CharacterAttributePtr, NULL, BufferLength);
		if (StringLengthPtr)
			*StringLengthPtr = 0;
		break;
	...
	case SQL_DESC_TYPE_NAME:
		...
		if (!ar->type_name) {
			...
			if (StringLengthPtr)
				*StringLengthPtr =
				    strlen((const char *)ar->type_name);
		}
#ifdef UNIX_DEBUG
		fprintf(stderr, "SQL_DESC_TYPE_NAME %s %d\n", __FILE__,
			__LINE__);
