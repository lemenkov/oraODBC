...
static char const rcsid[] =
    "$RCSfile: oracle_functions.c,v $ $Revision: 1.43 $";
...
void errcheck(char *file, int line, sword ret, OCIError * err)
{
	...
	case OCI_ERROR:
		OCIErrorGet((dvoid *) err, 1, NULL, &errnum, (text *) errtxt,
			    sizeof(errtxt), OCI_HTYPE_ERROR);
...
SQLRETURN ood_driver_error(void *hH, sword ret, char *file, int line)
{
	...
	case OCI_SUCCESS_WITH_INFO:
		{
			SQLRETURN result =
			    OCI_ERROR ? SQL_ERROR : SQL_SUCCESS_WITH_INFO;
			ub4 handle_type;
			text txt[512] = "";
			sb4 errcodep;
			int i = 1;
			...
			while (OCI_SUCCESS == OCIErrorGet(ood_driver_handle,
				...
				switch (errcodep) {
				case 1005:
					ood_post_diag((hgeneric *) hH,
						      ERROR_ORIGIN_08004, 0,
						      NULL,
						      "Server will not accept NULL password",
						      errcodep, 0, server_name,
						      ERROR_STATE_08004, file,
						      line);
					break;

				case 12154:
					ood_post_diag((hgeneric *) hH,
						      ERROR_ORIGIN_08001, 0,
						      NULL,
						      "TNS: Could not resolve TARGETDB parameter as service name",
						      errcodep, 0, server_name,
						      ERROR_STATE_08001, file,
						      line);
					break;
				...
				case 4043:
					ood_post_diag((hgeneric *) hH,
						      ERROR_ORIGIN_HY000, 0,
						      NULL, "Object not found",
						      errcodep, 0, server_name,
						      ERROR_STATE_HY000, file,
						      line);
					break;
...
*/
				...
				case 1406:
					ood_post_diag((hgeneric *) hH,
						      ...
						      NULL, ERROR_MESSAGE_01004,
						      errcodep, 0, server_name,
						      ERROR_STATE_01004, file,
						      line);
					break;

				case 910:
					ood_post_diag((hgeneric *) hH,
						      ERROR_ORIGIN_42000, 0,
						      NULL, ERROR_MESSAGE_42000,
						      errcodep, 0, server_name,
						      ERROR_STATE_42000, file,
						      line);
					break;

				case 3113:
					ood_post_diag((hgeneric *) hH,
						      ERROR_ORIGIN_08S01, 0,
						      NULL, ERROR_MESSAGE_08S01,
						      errcodep, 0, server_name,
						      ERROR_STATE_08S01, file,
						      line);
...
SQLRETURN ood_driver_connect(hDbc_T * dbc)
{
	...
	if (!gOCIEnv_p) {
		...
		OCIEnvCreate_log_stat(&gOCIEnv_p, OCI_THREADED | OCI_OBJECT,
				      0, 0, 0, 0, 0, 0, ret);
#else
		OCIInitialize_log_stat(OCI_OBJECT | OCI_THREADED, 0, 0, 0, 0,
				       ret);
	...
	ret =
	    OCIHandleAlloc_log_stat((dvoid *) gOCIEnv_p,
				    (dvoid **) & dbc->oci_err, OCI_HTYPE_ERROR,
				    0, 0, ret);
	if (ret) {
		/*
	...
	ret =
	    OCIHandleAlloc_log_stat((dvoid *) gOCIEnv_p,
				    (dvoid **) & dbc->oci_srv, OCI_HTYPE_SERVER,
				    0, 0, ret);
	if (ret) {
		THREAD_MUTEX_UNLOCK(dbc);
	...
	ret =
	    OCIServerAttach_log_stat(dbc->oci_srv, dbc->oci_err,
				     (text *) dbc->DB, strlen(dbc->DB),
				     (ub4) OCI_DEFAULT, ret);
	if (ret) {
		THREAD_MUTEX_UNLOCK(dbc);
		ood_driver_error(dbc, ret, __FILE__, __LINE__);
		...
		ret =
		    OCIAttrSet_log_stat(dbc->oci_svc, OCI_HTYPE_SVCCTX,
					dbc->oci_srv, 0, OCI_ATTR_SERVER,
					dbc->oci_err, ret);

		ret |=
		    OCIAttrSet_log_stat(dbc->oci_ses, OCI_HTYPE_SESSION,
					dbc->UID, strlen(dbc->UID),
					OCI_ATTR_USERNAME, dbc->oci_err, ret);

		ret |=
		    OCIAttrSet_log_stat(dbc->oci_ses, OCI_HTYPE_SESSION,
					dbc->PWD, strlen(dbc->PWD),
					OCI_ATTR_PASSWORD, dbc->oci_err, ret);
		if (ret) {
			THREAD_MUTEX_UNLOCK(dbc);
			ood_driver_error(dbc, ret, __FILE__, __LINE__);
			THREAD_MUTEX_LOCK(dbc);
		...
		ret =
		    OCISessionBegin_log_stat(dbc->oci_svc, dbc->oci_err,
					     dbc->oci_ses, OCI_CRED_RDBMS,
					     OCI_DEFAULT, ret);
#ifdef UNIX_DEBUG
		...
		ret =
		    OCIAttrSet_log_stat(dbc->oci_svc, OCI_HTYPE_SVCCTX,
					dbc->oci_ses, 0, OCI_ATTR_SESSION,
					dbc->oci_err, ret);
		if (ret) {
			THREAD_MUTEX_UNLOCK(dbc);
			ood_driver_error(dbc, ret, __FILE__, __LINE__);
...
SQLRETURN ood_driver_disconnect(hDbc_T * dbc)
{
	sword ret;
	ret =
	    OCISessionEnd_log_stat(dbc->oci_svc, dbc->oci_err, dbc->oci_ses, 0,
				   ret);
...
SQLRETURN ood_driver_prepare(hStmt_T * stmt, SQLCHAR * sql_in)
{
	...
	ret =
	    OCIHandleAlloc_log_stat((dvoid *) gOCIEnv_p,
				    (dvoid **) & stmt->oci_stmt, OCI_HTYPE_STMT,
				    0, 0, ret);
	if (ret) {
		if (ENABLE_TRACE) {
	...
	if (ret == OCI_SUCCESS || ret == OCI_SUCCESS_WITH_INFO) {
		...
		if (stmt->stmt_type == OCI_STMT_SELECT) {
			if (stmt->row_array_size) {
				status =
				    OCIStmtExecute_log_stat(stmt->dbc->oci_svc,
							    stmt->oci_stmt,
							    stmt->dbc->oci_err,
							    stmt->
							    row_array_size, 0,
...
SQLRETURN ood_driver_execute_describe(hStmt_T * stmt)
{
	...
	if (SQL_SUCCESS !=
	    ood_alloc_col_desc(stmt, num_cols, stmt->current_ir,
			       stmt->current_ar)) {
		return SQL_ERROR;
	}
	...
	for (i = 1; i <= num_cols; i++) {
		 ...
		 */
		ret =
		    OCIParamGet_log_stat((dvoid *) stmt->oci_stmt,
					 OCI_HTYPE_STMT, stmt->dbc->oci_err,
					 (dvoid *) & parm, i, ret);
		...
		if (!stmt->current_ir->recs.ir[i].data_type)
			ret |=
			    OCIAttrGet_log_stat((dvoid *) parm, OCI_DTYPE_PARAM,
		...
		if (!*stmt->current_ar->recs.ar[i].column_name) {
			siz =
			    siz >
			    ORACLE_MAX_COLUMN_LEN ? ORACLE_MAX_COLUMN_LEN : siz;
			memcpy(stmt->current_ar->recs.ar[i].column_name,
			       col_name, (int)siz);
		...
		if (precision == 0
		    && stmt->current_ir->recs.ir[i].data_type == SQLT_NUM)
			stmt->current_ar->recs.ar[i].precision = 38;
		else
			stmt->current_ar->recs.ar[i].precision =
		...
		status |=
		    ood_driver_setup_fetch_env(&stmt->current_ir->recs.ir[i],
...
SQLRETURN ood_ocitype_to_sqltype_imp(hStmt_T * stmt, int colNum)
{
	...
	if (!ret)
		ret =
		    ood_ocitype_to_sqltype(stmt->current_ir->recs.ir[colNum].
					   orig_type);
	...
	if (imp->num_recs) {
		...
		if (!imp->recs.ip) {
			ood_post_diag((hgeneric *) stmt, ERROR_ORIGIN_HY001, 0,
				      "", ERROR_MESSAGE_HY001, __LINE__, 0,
				      stmt->dbc->DSN, ERROR_STATE_HY001,
				      __FILE__, __LINE__);
			return SQL_ERROR;
		}

		app->recs.ap =
		    ORAREALLOC(app->recs.ap, sizeof(ap_T) * (param_num + 1));
		if (!app->recs.ap) {
			ood_post_diag((hgeneric *) stmt, ERROR_ORIGIN_HY001, 0,
				      "", ERROR_MESSAGE_HY001, __LINE__, 0,
				      stmt->dbc->DSN, ERROR_STATE_HY001,
		...
		imp->recs.ip =
		    ORAMALLOC(sizeof(struct ip_TAG) * (param_num + 1));
		if (!imp->recs.ip) {
			ood_post_diag((hgeneric *) stmt, ERROR_ORIGIN_HY001, 0,
				      "", ERROR_MESSAGE_HY001, __LINE__, 0,
				      stmt->dbc->DSN, ERROR_STATE_HY001,
		...
		app->recs.ap =
		    ORAMALLOC(sizeof(struct ap_TAG) * (param_num + 1));
		if (!app->recs.ap) {
			ood_post_diag((hgeneric *) stmt, ERROR_ORIGIN_HY001, 0,
				      "", ERROR_MESSAGE_HY001, __LINE__, 0,
				      stmt->dbc->DSN, ERROR_STATE_HY001,
...
SQLRETURN ood_driver_define_col(ir_T * ir)
{
			...
			ir->data_ptr = ORAMALLOC(ir->data_size
						 *
						 (ir->desc->stmt->
						  row_array_size));
			if (!ir->data_ptr) {
				ood_post_diag((hgeneric *) ir->desc->stmt->
					      current_ir, ERROR_ORIGIN_HY001, 0,
					      "", ERROR_MESSAGE_HY001, __LINE__,
					      0, ir->desc->stmt->dbc->DSN,
	...
	ir->length_arr =
	    ORAMALLOC(ir->desc->stmt->row_array_size * sizeof(ub2));
	...
	ret = OCIDefineByPos(ir->desc->stmt->oci_stmt, &dfn,
			     ...
			     ir->data_type, ir->ind_arr, ir->length_arr,
			     ir->rcode_arr, OCI_DEFAULT);
	if (ret) {
...
SQLRETURN(*ood_fn_default_copy(ub2 drvtype, SQLSMALLINT sqltype))
	...
	case SQLT_IBFLOAT:
		...
		case SQL_REAL:
			return (SQLRETURN(*)
				(int, ir_T *, SQLPOINTER, SQLINTEGER,
				 SQLINTEGER *))
			    ocibflt_sqlflt;
...
SQLRETURN ood_driver_setup_fetch_env(ir_T * ir, ar_T * ar)
{
	...
	if (!ar->data_type) {	/* not bound (yet) */
		...
		ar->display_size =
		    sqltype_display_size(ar->data_type, ir->data_size);
	}
	if (!ar->octet_length)
...
SQLRETURN ocistr_sqlnts(int row, ir_T * ir, SQLPOINTER target,
			SQLINTEGER buflen, SQLINTEGER * indi)
{
	...
	if (i < buflen) {
		...
		ood_post_diag((hgeneric *) ir->desc->stmt, ERROR_ORIGIN_01004,
			      ir->col_num, "", ERROR_MESSAGE_01004, __LINE__, 0,
			      "", ERROR_STATE_01004, __FILE__, __LINE__);
		if (indi) {
			indi[row] = buflen;
...
SQLRETURN ocibflt_sqlflt(int row, ir_T * ir, SQLPOINTER target,
			 SQLINTEGER buflen, SQLINTEGER * indi)
{
	int i;
	int ret;
...
SQLRETURN ocibflt_sqldouble(int row, ir_T * ir, SQLPOINTER target,
			    SQLINTEGER buflen, SQLINTEGER * indi)
{
	int i;
	int ret;
...
SQLRETURN ocibdbl_sqlfloat(int row, ir_T * ir, SQLPOINTER target,
			   SQLINTEGER buflen, SQLINTEGER * indi)
{
	int i;
	int ret;
...
SQLRETURN ocibdbl_sqldouble(int row, ir_T * ir, SQLPOINTER target,
			    SQLINTEGER buflen, SQLINTEGER * indi)
{
	int i;
	int ret;
...
SQLRETURN ocistr_memcpy(int row, ir_T * ir, SQLPOINTER target,
			SQLINTEGER buflen, SQLINTEGER * indi)
{
	int i;
	SQLCHAR *src;
...
SQLRETURN ocivnu_sqlnts(int row, ir_T * ir, SQLPOINTER target,
			SQLINTEGER buflen, SQLINTEGER * indi)
{
	...
	ret =
	    OCINumberToText(ir->desc->dbc->oci_err, (OCINumber *) ir->data_ptr,
#ifdef LIBCLNTSH8
			    (unsigned char *)"TM", 2,
#else
	...
	if (ENABLE_TRACE)
		ood_log_message(ir->desc->dbc, __FILE__, __LINE__,
				TRACE_FUNCTION_EXIT, (SQLHANDLE) ir->desc->stmt,
				0, "siss", NULL, "ocivnu_sqlnts",
				"BufferLength", buflen, "Source", c1, "Target",
				target);
 ...
 *  gcivnu_sqlslong
 */
SQLRETURN ocivnu_sqlslong(int row, ir_T * ir, SQLPOINTER target,
			  SQLINTEGER buflen, SQLINTEGER * indi)
{
	SQLCHAR *src;
	sword ret;
	...
	ood_log_message(ir->desc->dbc, __FILE__, __LINE__, TRACE_FUNCTION_ENTRY,
			...
			NULL, "ocivnu_sqlslong", "BufferLength", buflen);
#endif
	src = ((SQLCHAR *) ir->data_ptr) + (row * ir->data_size);

	ret = OCINumberToInt(ir->desc->dbc->oci_err,
			     (OCINumber *) src, local_min(buflen, sizeof(long)),
			     OCI_NUMBER_SIGNED, target);
	if (ret) {
		ood_driver_error(ir->desc->stmt, ret, __FILE__, __LINE__);
		return SQL_ERROR;
	}
 ...
 *  ocivnu_sqlulong
 */
SQLRETURN ocivnu_sqlulong(int row, ir_T * ir, SQLPOINTER target,
			  SQLINTEGER buflen, SQLINTEGER * indi)
{
	SQLCHAR *src;
	sword ret;
 ...
 *  ocivnu_sqldouble
 */
SQLRETURN ocivnu_sqldouble(int row, ir_T * ir, SQLPOINTER target,
			   SQLINTEGER buflen, SQLINTEGER * indi)
{
	SQLCHAR *src;
	sword ret;
	...
	ret = OCINumberToReal(ir->desc->dbc->oci_err,
			      (OCINumber *) src, sizeof(double), target);
	if (ret) {
		ood_driver_error(ir->desc->stmt, ret, __FILE__, __LINE__);
		return SQL_ERROR;
	}
 ...
 *  ocivnu_sqlfloat
 */
SQLRETURN ocivnu_sqlfloat(int row, ir_T * ir, SQLPOINTER target,
			  SQLINTEGER buflen, SQLINTEGER * indi)
{
	SQLCHAR *src;
	sword ret;
	src = ((SQLCHAR *) ir->data_ptr) + (row * ir->data_size);
	ret = OCINumberToReal(ir->desc->dbc->oci_err,
			      (OCINumber *) src, sizeof(float), target);
	if (ret) {
		ood_driver_error(ir->desc->stmt, ret, __FILE__, __LINE__);
		return SQL_ERROR;
	}
 ...
 *  ocivnu_sqlsshort
 */
SQLRETURN ocivnu_sqlsshort(int row, ir_T * ir, SQLPOINTER target,
			   SQLINTEGER buflen, SQLINTEGER * indi)
{
	SQLCHAR *src;
	sword ret;
	...
	ood_log_message(ir->desc->dbc, __FILE__, __LINE__, TRACE_FUNCTION_EXIT,
			...
			"BufferLength", buflen, "Target", target);
#endif
	return SQL_SUCCESS;
}
 ...
 *  ocivnu_sqlushort
 */
SQLRETURN ocivnu_sqlushort(int row, ir_T * ir, SQLPOINTER target,
			   SQLINTEGER buflen, SQLINTEGER * indi)
{
	SQLCHAR *src;
	sword ret;
	...
	ret = OCINumberToInt(ir->desc->dbc->oci_err,
			     (OCINumber *) src, sizeof(unsigned short),
			     OCI_NUMBER_UNSIGNED, target);
	if (ret) {
		ood_driver_error(ir->desc->stmt, ret, __FILE__, __LINE__);
		return SQL_ERROR;
			     ...
			     SQLINTEGER buflen, SQLINTEGER * indi)
{
	...
	ret = OCINumberToInt(ir->desc->dbc->oci_err,
			     (OCINumber *) ir->data_ptr, sizeof(char),
			     OCI_NUMBER_SIGNED, target);
	if (ret) {
		ood_driver_error(ir->desc->stmt, ret, __FILE__, __LINE__);
		return SQL_ERROR;
			...
			SQLINTEGER buflen, SQLINTEGER * indi)
{
	...
	if (buflen < 2) {
		ood_post_diag((hgeneric *) ir->desc->stmt, ERROR_ORIGIN_22003,
			      ...
			      __LINE__, 0, "", ERROR_STATE_22003, __FILE__,
			      __LINE__);
		return SQL_ERROR;
	}
 ...
 *  ociflt_sqlnts
 */
SQLRETURN ociflt_sqlnts(int row, ir_T * ir, SQLPOINTER target,
			SQLINTEGER buflen, SQLINTEGER * indi)
{
	SQLCHAR *src;
	src = ((SQLCHAR *) ir->data_ptr) + (row * ir->data_size);
 ...
 *  ociflt_sqlfloat
 */
SQLRETURN ociflt_sqlfloat(int row, ir_T * ir, SQLPOINTER target,
			  SQLINTEGER buflen, SQLINTEGER * indi)
{
 ...
 *  ocidat_sqlnts
 */
SQLRETURN ocidat_sqlnts(int row, ir_T * ir, SQLPOINTER target,
			SQLINTEGER buflen, SQLINTEGER * indi)
{
	SQLCHAR *src;
	sword ret;
	...
	ood_log_message(ir->desc->dbc, __FILE__, __LINE__, TRACE_FUNCTION_EXIT,
			...
			NULL, "ocidat_sqlnts",
			"BufferLength", buflen, "&Target", (long)target);
#endif
			 ...
			 SQLINTEGER buflen, SQLINTEGER * indi)
{
	...
	OCIDateGetTime((OCIDate *) src, (ub1 *) & h, (ub1 *) & m, (ub1 *) & s);
	ts->hour = (SQLUSMALLINT) h;
	ts->minute = (SQLUSMALLINT) m;
	ts->second = (SQLUSMALLINT) s;
	if (indi)
			...
			SQLINTEGER buflen, SQLINTEGER * indi)
{
	...
	OCILobGetLength(ir->desc->stmt->dbc->oci_svc,
			ir->desc->stmt->dbc->oci_err, ir->locator[row],
			&ir->lobsiz);
	...
	ret =
	    OCILobRead(ir->desc->stmt->dbc->oci_svc,
		       ir->desc->stmt->dbc->oci_err, ir->locator[row], &amtp,
		       ir->posn, target, (ub4) ir->lobsiz, NULL, NULL, 0,
		       SQLCS_IMPLICIT);
	if (ret == OCI_ERROR) {
		ood_driver_error(ir->desc->stmt, ret, __FILE__, __LINE__);
		if (indi)
	...
	ood_log_message(ir->desc->dbc, __FILE__, __LINE__, TRACE_FUNCTION_EXIT,
			...
			NULL, "ocilob_sqllvc", "Target", target);
#endif
	if (ret != OCI_ERROR) {
			...
			SQLINTEGER buflen, SQLINTEGER * indi)
{
	...
	ood_log_message(ir->desc->dbc, __FILE__, __LINE__, TRACE_FUNCTION_ENTRY,
			...
			"ir->data_ptr", ir->data_ptr, "&Target", (long)target);
#endif
	...
	if (!strncmp((char *)str, "BLOB", 4)
	    || !strncmp((char *)str, "LONG RAW", 9)) {
	...
	if (!strncmp((char *)str, "CLOB", 4)
	    || !strncmp((char *)str, "LONG", 4)) {
 ...
 *  ocidty_sqlnts
 */
SQLRETURN ocidty_sqlnts(int row, ir_T * ir, SQLPOINTER target,
			SQLINTEGER buflen, SQLINTEGER * indi)
{
	int i;
...
sword ood_driver_bind_param(hStmt_T * stmt, ub4 parmnum)
{
	...
	if (bind_indicator < 0 && bind_indicator != SQL_NTS) {
		...
		if (debugLevel() > 0) {
			printf
			    ("ood_driver_bind_param::line %d concise_type=%d %s\n",
			     __LINE__, ap->concise_type,
			     odbc_var_type(ap->concise_type));
		}
		...
		case SQL_C_TIMESTAMP:
			{
				...
				SQL_TIMESTAMP_STRUCT *ts =
				    (SQL_TIMESTAMP_STRUCT *) ap->data_ptr;
				ip->data_ptr =
				    (OCIDate *) ORAMALLOC(sizeof(OCIDate));
				...
				case SQL_TYPE_TIMESTAMP:
					sprintf(datetxt,
						...
						ts->second);
					ret =
					    OCIDateFromText(stmt->dbc->oci_err,
							    (text *) datetxt,
							    14,
							    (text *)
							    "YYYYMMDDHH24MISS",
		...
		case SQL_C_TIME:
			{
				...
				ret = OCIDateFromText(stmt->dbc->oci_err,
						      ...
						      (OCIDate *) ip->data_ptr);
				if (ret) {
					ood_driver_error(stmt, ret, __FILE__,
							 __LINE__);
					return (ret);
				}
			}
	...
	ood_log_message(ip->desc->dbc, __FILE__, __LINE__, TRACE_FUNCTION_EXIT,
			...
			"bind_mode", bind_mode, "bind_ptr", bind_ptr);
#endif
	if (debugLevel() > 0) {
		printf
		    ("ood_driver_bind_param::OCIBindByPos parm=%d size=%d type=%d %s\n",
		     parmnum, ip->data_size, ip->data_type,
		     oci_var_type(ip->data_type));
	}
	return (ret);
}
...
char *oci_hdtype_name(ub4 hdtype)
{
	...
	case OCI_DTYPE_SNAP:
		return "OCI_DTYPE_SNAP";
	case OCI_DTYPE_RSET:
		return "OCI_DTYPE_RSET";
	case OCI_DTYPE_PARAM:
		return "OCI_DTYPE_PARAM";
	case OCI_DTYPE_ROWID:
		return "OCI_DTYPE_ROWID";
	case OCI_DTYPE_COMPLEXOBJECTCOMP:
		return "OCI_DTYPE_COMPLEXOBJECTCOMP";
	case OCI_DTYPE_FILE:
