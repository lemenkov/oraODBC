...
void LogODBCError(SQLRETURN nResult, SQLSMALLINT fHandleType, SQLHANDLE handle);
int db_connect();
char *Get_DSNName()
{
	return (char *)dsn;
...
int db_connect()
{
	...
	nResult =
	    SQLAllocHandle(SQL_HANDLE_DBC, EnvHandle, (SQLHDBC *) & ConHandle);
...
void LogODBCError(SQLRETURN nResult, SQLSMALLINT fHandleType, SQLHANDLE handle)
{
	...
	char szBuffer[SQL_SQLSTATE_SIZE + SQL_MAX_MESSAGE_LENGTH + MAXBUFLEN +
		      1];
	...
	while (SQL_SUCCEEDED(nErrResult)) {
		...
		nErrResult =
		    SQLGetDiagRec(fHandleType, handle, sMsgNum++, szErrState,
				  &dwErrCode, szErrText,
		...
		if (SQL_SUCCEEDED(nErrResult)) {
			...
			printf
			    ("Unable to retrieve ODBC error information, code %d\n",
			     nErrResult);
		}
