...
int main()
{
	...
	for (col_usi = 1; col_usi <= numCols_si; col_usi++) {
		rc = SQLDescribeCol(StmtHandle,
				    ...
				    sizeof(buf1_cp),
				    &len_si,
				    &type_si, &sz_ui, &scale_si, &nullable_si);

		assert(rc == SQL_SUCCESS);

		VERBOSE
		    ("SQLDescribeCol col=%d name:%s len=%d type=%d size=%d scale_si=%d nullable=%d\n",
		     col_usi, buf1_cp, len_si, type_si, sz_ui, scale_si,
		     nullable_si);

		if (col_usi <= 3)
			assert(type_si == SQL_INTEGER);
		if (col_usi == 4 || col_usi == 5)
			assert(type_si == SQL_NUMERIC);
		if (col_usi == 6)
			assert(type_si == SQL_DOUBLE);
		if (col_usi >= 7)
			assert(type_si == SQL_FLOAT);
		...
		if (col_usi <= 3)
			assert(type_i == SQL_INTEGER);
		if (col_usi == 4 || col_usi == 5)
			assert(type_i == SQL_NUMERIC);
		if (col_usi == 6)
			assert(type_i == SQL_DOUBLE);
		if (col_usi >= 7)
			assert(type_i == SQL_FLOAT);
		...
		rc = SQLColAttribute(StmtHandle, col_usi, SQL_DESC_TYPE_NAME,
				     buf2_cp, MAX_LEN, &sz_si,
				     (SQLPOINTER) NULL);

		assert(rc == SQL_SUCCESS);
		VERBOSE("col %d DESC_TYPE_NAME= %s\n", col_usi, buf2_cp);
		if (col_usi < 4)
			assert(strcmp(buf2_cp, "INTEGER") == 0);
		if (col_usi == 4 || col_usi == 5)
			assert(strcmp(buf2_cp, "NUMBER") == 0);
		if (col_usi > 5)
			assert(strcmp(buf2_cp, "FLOAT") == 0);
	...
	while (SQLFetch(StmtHandle) == SQL_SUCCESS) {
		...
		VERBOSE
		    ("anInteger_i=%d, anInt_i=%d, aSmallInt_i=%d, aDecimal83_f=%f, aNumeric94_f=%f, aFloat_f=%f, aFloat9_f=%f, aReal_f=%f\n",
